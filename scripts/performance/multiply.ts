...
const bench = (): any => {
  ...
  for (const [algorithm, fnc] of Object.entries(algorithmMapper)) {
    const startTime = performance.now()
    for (let i = 0; i < EXECUTION_COUNT; ++i)baseMultiply(UNSAFE_NEG_ARG_A, UNSAFE_NEG_ARG_B, fnc)
    const endTime = performance.now()
    console.log(`\t\t${algorithm} on ${EXECUTION_COUNT} executions : ${endTime - startTime} milliseconds`)
  }
  ...
  for (const [algorithm, fnc] of Object.entries(algorithmMapper)) {
    const startTime = performance.now()
    for (let i = 0; i < EXECUTION_COUNT; ++i)baseMultiply(UNSAFE_NEG_ARG_A, UNSAFE_POS_ARG_B, fnc)
    const endTime = performance.now()
    console.log(`\t\t${algorithm} on ${EXECUTION_COUNT} executions : ${endTime - startTime} milliseconds`)
  }
